...
from ankigen_core.agents.integration import (
    AgentOrchestrator,
...
async def test_agent_orchestrator_initialize_success(
    mock_get_flags, mock_client_manager, mock_openai_client, enabled_feature_flags
):
    ...
    with patch(
        "ankigen_core.agents.integration.GenerationCoordinator"
    ) as mock_gen_coord, patch(
        "ankigen_core.agents.integration.JudgeCoordinator"
    ) as mock_judge_coord, patch(
        "ankigen_core.agents.integration.RevisionAgent"
    ) as mock_revision, patch(
        "ankigen_core.agents.integration.EnhancementAgent"
    ) as mock_enhancement:
        orchestrator = AgentOrchestrator(mock_client_manager)
...
@patch("ankigen_core.agents.integration.get_feature_flags")
async def test_agent_orchestrator_initialize_partial_flags(
    mock_get_flags, mock_client_manager, mock_openai_client
):
...
async def test_generate_cards_with_agents_success(
    mock_record,
    mock_get_flags,
    mock_client_manager,
    sample_cards,
    ...
    orchestrator._quality_phase = AsyncMock(
        return_value=(sample_cards, {"quality": "good"})
    )
...
@patch("ankigen_core.agents.integration.get_feature_flags")
async def test_generate_cards_with_agents_not_enabled(
    mock_get_flags, mock_client_manager
):
...
@patch("ankigen_core.agents.integration.get_feature_flags")
@patch("ankigen_core.agents.integration.record_agent_execution")
async def test_generate_cards_with_agents_error(
    mock_record, mock_get_flags, mock_client_manager, enabled_feature_flags
):
    ...
    orchestrator = AgentOrchestrator(mock_client_manager)
    orchestrator.openai_client = MagicMock()
    orchestrator._generation_phase = AsyncMock(
...
async def test_generation_phase_with_coordinator(
    mock_subject_expert, mock_client_manager, sample_cards, enabled_feature_flags
):
...
@patch("ankigen_core.agents.integration.SubjectExpertAgent")
async def test_generation_phase_with_subject_expert(
    mock_subject_expert, mock_client_manager, sample_cards
):
    ...
    result = await orchestrator._generation_phase(
        topic="Python", subject="programming", num_cards=2, difficulty="beginner"
    )

    assert result == sample_cards
    mock_subject_expert.assert_called_once_with(
        orchestrator.openai_client, "programming"
    )
    mock_expert_instance.generate_cards.assert_called_once_with(
        topic="Python", num_cards=2, difficulty="beginner", context=None
    )
...
# Test _quality_phase
async def test_quality_phase_success(
    mock_client_manager, sample_cards, enabled_feature_flags
):
    ...
    mock_judge_coordinator.coordinate_judgment.side_effect = [
        judge_results,
        re_judge_results,
    ]
...
async def test_quality_phase_revision_fails(
    mock_client_manager, sample_cards, enabled_feature_flags
):
    ...
    mock_revision_agent = MagicMock()
    mock_revision_agent.revise_card = AsyncMock(
...
async def test_enhancement_phase_success(mock_client_manager, sample_cards):
    ...
    mock_enhancement_agent.enhance_card_batch.assert_called_once_with(
        cards=sample_cards, enhancement_targets=["explanation", "example", "metadata"]
    )
...
def test_get_performance_metrics(
    mock_get_metrics, mock_client_manager, enabled_feature_flags
):
...
async def test_integrate_with_existing_workflow_agents_enabled(
    ...
    mock_orchestrator.generate_cards_with_agents = AsyncMock(
        return_value=(sample_cards, {"test": "metadata"})
    )
    ...
    mock_orchestrator.generate_cards_with_agents.assert_called_once_with(
        topic="Python", subject="programming"
    )


@patch("ankigen_core.agents.integration.get_feature_flags")
async def test_integrate_with_existing_workflow_agents_disabled(
    mock_get_flags, mock_client_manager
):
    ...
    with pytest.raises(NotImplementedError, match="Legacy fallback not implemented"):
        await integrate_with_existing_workflow(
            client_manager=mock_client_manager, api_key="test-key", topic="Python"
        )


# Integration tests
@patch("ankigen_core.agents.integration.get_feature_flags")
async def test_full_agent_workflow_integration(
    mock_get_flags, mock_client_manager, sample_cards, enabled_feature_flags
):
    ...
    with patch(
        ...
        "ankigen_core.agents.integration.EnhancementAgent"
    ) as mock_enhancement, patch(
        "ankigen_core.agents.integration.record_agent_execution"
    ) as mock_record:
        ...
        judge_results = [
            (card, ["decision"], True) for card in sample_cards
        ]  # All approved
        ...
        mock_enhancement_instance.enhance_card_batch = AsyncMock(
...
async def test_orchestrator_handles_generation_timeout(
    mock_client_manager, enabled_feature_flags
):
    ...
    orchestrator.openai_client = MagicMock()
    orchestrator._generation_phase = AsyncMock(
        side_effect=asyncio.TimeoutError("Generation timed out")
    )
...
async def test_orchestrator_handles_quality_phase_error(
    mock_client_manager, sample_cards, enabled_feature_flags
):
...
async def test_orchestrator_handles_enhancement_error(
    mock_client_manager, sample_cards, enabled_feature_flags
):
    ...
    orchestrator._enhancement_phase = AsyncMock(
