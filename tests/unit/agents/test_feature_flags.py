...
def test_should_use_agents_hybrid_mode_with_generation_agent():
    ...
    flags = AgentFeatureFlags(mode=AgentMode.HYBRID, enable_subject_expert_agent=True)
    assert flags.should_use_agents() is True


def test_should_use_agents_hybrid_mode_with_judge_agent():
    """Test should_use_agents() in HYBRID mode with judge agent enabled"""
    flags = AgentFeatureFlags(mode=AgentMode.HYBRID, enable_content_accuracy_judge=True)
    assert flags.should_use_agents() is True
...
def test_should_use_agents_ab_test_mode_without_hash():
    ...
    flags = AgentFeatureFlags(mode=AgentMode.A_B_TEST, ab_test_ratio=0.5)
...
def test_get_feature_flags_first_call():
    ...
    with patch(
        "ankigen_core.agents.feature_flags.AgentFeatureFlags.from_env"
    ) as mock_from_env:
...
def test_feature_flags_ab_test_consistency():
    ...
    flags = AgentFeatureFlags(
        mode=AgentMode.A_B_TEST, ab_test_ratio=0.5, ab_test_user_hash="consistent_user"
    )
